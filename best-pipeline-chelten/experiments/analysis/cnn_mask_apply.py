...
with open("/tmp/cnn_dataset.pkl","rb") as f:
    D = pickle.load(f)
cand_t = D["t"]; cand_label = D["label"]; snips = D["snips"]

with open("/tmp/cnn_train_result.pkl","rb") as f:
    TR = pickle.load(f)
scale = TR["scale"]; n_train = TR["n_train"]
...
model = SCGNet()
model.load_state_dict(torch.load("/tmp/cnn_model.pt"))
model.eval()

with torch.no_grad():
    Xall = torch.tensor(snips/scale, dtype=torch.float32).unsqueeze(1)
    cand_cnn = torch.sigmoid(model(Xall)).numpy().ravel()
print(f"scored {len(cand_cnn)} candidates with the CNN (train-set candidates included, "
      f"consistent with how the NCC template was applied globally in v9)")
...
with open("/tmp/final_4way_compare.pkl","rb") as f:
    Dc = pickle.load(f)
LAG_US = Dc["LAG_US"]; RESTRICT = (Dc["t0"], Dc["t1"])
...
s_h = stats(xa_h, yb_h)
print(f"\nHAND baseline:  n={s_h['n']}  r={s_h['r']:.4f}  MAE={s_h['mae']:.2f}")
...
# same config grid as v9's shape mask, for apples-to-apples comparison
configs = [
    ("cutoff=0.5 gap=2.0s min_n=2",  0.5, 2.0, 2),
    ("cutoff=0.6 gap=2.0s min_n=2",  0.6, 2.0, 2),
    ("cutoff=0.7 gap=2.0s min_n=2",  0.7, 2.0, 2),
    ("cutoff=0.8 gap=2.0s min_n=2",  0.8, 2.0, 2),
    ("cutoff=0.9 gap=2.0s min_n=2",  0.9, 2.0, 2),
    ("cutoff=0.7 gap=1.5s min_n=2",  0.7, 1.5, 2),
...
for name, cutoff, gap_max_s, min_n in configs:
    ...
    agree = (hv==av).mean()
    print(f"{name:32s}  n={s_a['n']:5d}  r={s_a['r']:.4f}  MAE={s_a['mae']:.2f}  bias={s_a['bias']:+.2f}  beat_agree={agree*100:.1f}%")
    results[name] = dict(auto_ivs=auto_ivs, s_a=s_a, agree=agree, cutoff=cutoff, gap_max_s=gap_max_s, min_n=min_n)

with open("/tmp/cnn_mask_apply.pkl","wb") as f:
    pickle.dump(dict(results=results, s_h=s_h, scg_tmid=scg_tmid, scg_hr_sm=scg_hr_sm,
                      scg_v_hand=scg_v_hand, LAG_US=LAG_US, RESTRICT=RESTRICT,
                      cand_t=cand_t, cand_cnn=cand_cnn, cand_label=cand_label), f)
print("\nsaved /tmp/cnn_mask_apply.pkl")
