...
with open(f"/tmp/extended_seg_{SEG}_envelope.pkl","rb") as f:
    E = pickle.load(f)
...
cand_t = ts_sd[pk_idx]
print(f"[{SEG}] primary candidates: {len(cand_t)}")
...
print(f"[{SEG}] main CNN confirmed: {len(confirmed0)}   rejected: {len(rejected_t)}")
...
print(f"[{SEG}] S2-flagged: {n_flagged}   recovered+accepted: {n_accepted}")
...
print(f"[{SEG}] final confirmed SCG beat set (pre-gap-aware): {len(merged_arr)}  over {(E['t1']-E['t0'])/1e6/60:.2f} min -> {len(merged_arr)/((E['t1']-E['t0'])/1e6/60):.1f} beats/min")

with open(f"/tmp/extended_seg_{SEG}_merged.pkl","wb") as f:
    pickle.dump(dict(merged_arr=merged_arr, cand_t=cand_t, n_flagged=n_flagged, n_accepted=n_accepted), f)
print(f"saved /tmp/extended_seg_{SEG}_merged.pkl")
