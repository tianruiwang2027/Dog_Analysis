...
print("n ECG hand clicks (both-good):", len(ecg_good))

scg_hand_w = scg_hand_pk[(scg_hand_pk>=t0)&(scg_hand_pk<=t1)]
scg_hand_good = scg_hand_w[both_good(scg_hand_w)]
scg_hand_tmid, scg_hand_hr_raw = beat_hr(scg_hand_good)
scg_hand_hr_sm = smooth(scg_hand_tmid, scg_hand_hr_raw)
print("n SCG hand clicks (both-good):", len(scg_hand_good))

singles_good = singles_good_all[both_good(singles_good_all)]
print("n auto singles (both-good):", len(singles_good))
...
def run_vs(name, peaks, ref_tmid, ref_hr_sm, ref_name):
    ...
    s = stats(xa,yb)
    print(f"{name:32s} vs {ref_name:10s} n_peaks={len(peaks):5d} n_pairs={s['n']:5d}  r={s['r']:.3f}  MAE={s['mae']:.2f}  bias={s['bias']:+.2f}")

print("\n--- gold standard benchmark ---")
run_vs("hand-clicked SCG", scg_hand_good, ecg_tmid, ecg_hr_sm, "hand ECG")

print("\n--- automated detector vs ECG ---")
run_vs("baseline (no filter)", singles_good, ecg_tmid, ecg_hr_sm, "hand ECG")
for cutoff in [0.5,0.6,0.65,0.7,0.75,0.8]:
    run_vs(f"wide-template cutoff={cutoff}", singles_good[scores>=cutoff], ecg_tmid, ecg_hr_sm, "hand ECG")

print("\n--- automated detector vs hand-clicked SCG (does it reproduce the human's SCG judgment?) ---")
run_vs("baseline (no filter)", singles_good, scg_hand_tmid, scg_hand_hr_sm, "hand SCG")
for cutoff in [0.5,0.6,0.65,0.7,0.75,0.8]:
    run_vs(f"wide-template cutoff={cutoff}", singles_good[scores>=cutoff], scg_hand_tmid, scg_hand_hr_sm, "hand SCG")
