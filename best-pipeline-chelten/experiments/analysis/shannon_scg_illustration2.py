...
with open(f"/tmp/shannon_restricted_results_thr{THR}.pkl","rb") as f:
    d = pickle.load(f)
...
axs[3].axvspan(dn(w0+int(0*1e6))[0], dn(w0+int(8*1e6))[0], color="red", alpha=0.06)
axs[3].text(dn(w0+int(0.3*1e6))[0], axs[3].get_ylim()[1]*0.9,
            "q995 still inflated by\nthe 17:47:02-04 burst here\n(sharpening suppressed)",
            fontsize=8, color="firebrick")
axs[3].set_ylabel("sharpened\n(local q995 + exp)")
axs[3].set_title(f"4) Local-q995-sharpened envelope -- peak-picking (thr={THR}) + S1/S2 pairing (160-200ms, >=150ms refractory)")
...
fig.suptitle(f"[Chelten] SCG Shannon-energy S1/S2 pipeline, threshold={THR} -- 17:47:00-17:47:16", fontsize=13)
fig.tight_layout()
out = "/tmp/dog-test-ecg/dog-test-ecg-code/figs/chelten_shannon_scg_illustration_lowthr.png"
