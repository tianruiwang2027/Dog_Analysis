...
print("\n--- CNN (envelope, SCG+ECG relabeled), held-out test (thr=0.5) ---")
...
torch.save(best_state, "/tmp/cnn_model_relabel.pt")
with open("/tmp/cnn_train_relabel_result.pkl","wb") as f:
    pickle.dump(dict(scale=scale, n_train=n_train, test_prob=test_prob, y_test=y_test, t_test=t[n_train:]), f)
print("saved /tmp/cnn_model_relabel.pt")
