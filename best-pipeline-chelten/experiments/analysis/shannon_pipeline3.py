#!/usr/bin/env python3
"""Fixed version: the envelope amplitude/contact quality drifts a LOT over
this 3.5-hour recording (confirmed: candidate-peak density was concentrated
in 16:09-17:24 and near-ZERO in our actual analysis window 17:26-18:00 when
using one GLOBAL percentile threshold). Fix: make both the q995 sharpening
scale and the peak-detection threshold LOCAL/rolling (a few-second window),
same spirit as Pan-Tompkins' rolling adaptive floor, so the detector adapts
to whatever the local signal amplitude is instead of being calibrated by a
different, unrelated part of the session."""
import glob, sqlite3
...
FSD = 200.0   # working rate after decimation (Hz) -- matches scg_hr.py's own choice
...
def pick_doublets(ts, env, fsd, refract_pk_s=0.08, s1s2_lo=0.16, s1s2_hi=0.20, post_s2_refract_s=0.15,
                   thr=0.6):
...
print("--- SCG: Shannon-energy envelope (LOCAL threshold) + S1/S2 doublet pairing ---")
...
print(f"  decimated to {fsd_s:.1f}Hz, n={len(ts_sd)}")
s1_scg, cand_scg = pick_doublets(ts_sd, sharp_s, fsd_s, s1s2_lo=0.14, s1s2_hi=0.28, thr=0.6)
...
print("--- ECG: same fixed envelope front-end, single-peak-per-refractory ---")
...
print(f"  decimated to {fsd_e:.1f}Hz, n={len(ts_ed)}")
...
with open("/tmp/shannon_full_results2.pkl","wb") as f:
