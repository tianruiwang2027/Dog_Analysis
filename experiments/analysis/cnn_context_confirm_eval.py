...
with open("/tmp/cnn_context_full_scores.pkl","rb") as f:
    CS = pickle.load(f)
cand_t = CS["cand_t"]; cand_ctx = CS["cand_ctx"]; LAG_US = CS["LAG_US"]; RESTRICT = CS["RESTRICT"]

with open("/tmp/cnn_mask_apply.pkl","rb") as f:
    CA = pickle.load(f)
cand_cnn_old = CA["cand_cnn"]   # same order/length as cand_t (both built from the same cand_t array)
...
def confirm_and_run(scores, cutoff, max_rr_s):
    confirmed = np.sort(cand_t[scores >= cutoff])
    ...
    s = stats(xa, yb)
    return dict(cutoff=cutoff, max_rr_s=max_rr_s, n_confirmed=len(confirmed), s=s)

print("=== ORIGINAL shape-only model ===")
best_old = None
for cutoff in [0.30,0.40,0.50,0.55,0.60,0.70,0.80]:
    for max_rr in [1.5]:
        r = confirm_and_run(cand_cnn_old, cutoff, max_rr)
        if r is None: continue
        print(f"  cutoff={cutoff:.2f}  n={r['s']['n']:5d}  r={r['s']['r']:.4f}  MAE={r['s']['mae']:.2f}")
        if best_old is None or r["s"]["r"]>best_old["s"]["r"]: best_old=r

print("\n=== NEW shape+context model ===")
best_new = None
for cutoff in [0.05,0.10,0.15,0.20,0.30,0.40,0.50,0.60,0.70]:
    for max_rr in [1.5]:
        r = confirm_and_run(cand_ctx, cutoff, max_rr)
        if r is None: continue
        print(f"  cutoff={cutoff:.2f}  n={r['s']['n']:5d}  r={r['s']['r']:.4f}  MAE={r['s']['mae']:.2f}")
        if best_new is None or r["s"]["r"]>best_new["s"]["r"]: best_new=r

print(f"\nBEST ORIGINAL: cutoff={best_old['cutoff']:.2f}  r={best_old['s']['r']:.4f}  n={best_old['s']['n']}")
print(f"BEST NEW:      cutoff={best_new['cutoff']:.2f}  r={best_new['s']['r']:.4f}  n={best_new['s']['n']}")

with open("/tmp/cnn_context_confirm_eval.pkl","wb") as f:
    pickle.dump(dict(best_old=best_old, best_new=best_new), f)
