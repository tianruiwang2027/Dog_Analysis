...
with open("/tmp/mask_v4_composite.pkl","rb") as f:
    M = pickle.load(f)
tc = M["tc"]; proba_cv = M["proba_cv"]; THR = M["THR"]

# --- build continuous auto-mask intervals from the per-window (1s step, 3s win) probabilities ---
# a window center tc is "auto-good" if proba_cv < THR (i.e. NOT flagged bad)
is_good = proba_cv < THR
order = np.argsort(tc)
tc_s = tc[order]; good_s = is_good[order]

STEP_US = int(1.0e6)  # matches STEP_S in mask_v3_features.py
auto_ivs = []
cur_start = None
for i in range(len(tc_s)):
    if good_s[i]:
        if cur_start is None:
            cur_start = tc_s[i] - STEP_US//2
    else:
        if cur_start is not None:
            auto_ivs.append((cur_start, tc_s[i]-STEP_US//2))
            cur_start = None
if cur_start is not None:
    auto_ivs.append((cur_start, tc_s[-1]+STEP_US//2))
print(f"auto-mask: {len(auto_ivs)} good intervals covering "
...
# --- reload hand-labeled good intervals (same as before) ---
...
ecg_v_at_scg = good_mask(scg_tmid, ecg_ivs)  # ECG-good, evaluated at SCG-clock times (same recording clock)
scg_v_hand = good_mask(scg_tmid, scg_ivs_hand) & ecg_v_at_scg
scg_v_auto = auto_good_mask(scg_tmid) & ecg_v_at_scg

print()
print("=== hand SCG vs ECG: HAND-labeled quality mask (baseline) vs AUTOMATED composite mask ===")
print(f"(restricted to the official comparison window {datetime.datetime.utcfromtimestamp(D['t0']/1e6)} - {datetime.datetime.utcfromtimestamp(D['t1']/1e6)} UTC)")
RESTRICT = (D["t0"], D["t1"])
xa_h, yb_h, ta_h = align(scg_tmid, scg_hr_sm, scg_v_hand, ecg_tmid, ecg_hr_sm, ecg_v, lag_us=LAG_US, restrict=RESTRICT)
s_h = stats(xa_h, yb_h)
print(f"HAND  mask:  n={s_h['n']:5d}  r={s_h['r']:.4f}  MAE={s_h['mae']:.2f}  bias={s_h['bias']:+.2f}")

xa_a, yb_a, ta_a = align(scg_tmid, scg_hr_sm, scg_v_auto, ecg_tmid, ecg_hr_sm, ecg_v, lag_us=LAG_US, restrict=RESTRICT)
s_a = stats(xa_a, yb_a)
print(f"AUTO  mask:  n={s_a['n']:5d}  r={s_a['r']:.4f}  MAE={s_a['mae']:.2f}  bias={s_a['bias']:+.2f}")

# agreement between hand and auto mask at the level of individual SCG beats, in the official window
...
both_bad = (~hv & ~av).sum()
print()
print(f"beat-level agreement (hand-good vs auto-good over all SCG beats, ECG-good-restricted): {agree*100:.1f}%")
print(f"  both good={both_good}  hand-good-only={hand_only}  auto-good-only={auto_only}  both bad={both_bad}")

with open("/tmp/mask_v4_apply_and_validate.pkl","wb") as f:
    pickle.dump(dict(auto_ivs=auto_ivs, scg_ivs_hand=scg_ivs_hand,
                      xa_h=xa_h, yb_h=yb_h, ta_h=ta_h, s_h=s_h,
                      xa_a=xa_a, yb_a=yb_a, ta_a=ta_a, s_a=s_a,
                      scg_tmid=scg_tmid, scg_hr_sm=scg_hr_sm, scg_v_hand=scg_v_hand, scg_v_auto=scg_v_auto,
                      LAG_US=LAG_US), f)
print("\nsaved /tmp/mask_v4_apply_and_validate.pkl")
