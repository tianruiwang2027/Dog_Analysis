...
for ax, thr in zip(axs.flat, thresholds):
    ...
    pts, pbpm, psqi, pv = load_coral(
        f"{HERE}/coral_ecg/Chelten_ecg_polar/out.csv",
        f"{HERE}/coral_ecg/Chelten_ecg_polar/mask.parquet", thr)
...
fig.suptitle("[Chelten] SCG vs Polar CORAL HR -- sweeping the band-specific SQI floor\n"
             "(ORIGINAL un-bridged mask + exact-repeat-run filter always on (no gap-bridging at all); orange = 95-125bpm attractor zone)",
             fontsize=12)
fig.tight_layout()
out = "/tmp/dog-test-ecg/dog-test-ecg-code/figs/chelten_band_sweep_nobridge.png"
