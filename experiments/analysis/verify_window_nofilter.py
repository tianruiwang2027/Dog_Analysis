#!/usr/bin/env python3
"""Verification: SCG vs Polar CORAL HR, no anti-coasting, no exact-repeat-run
...
def load_coral(path, maskpath=None):
    ...
    if maskpath and os.path.exists(maskpath):
        ...
        for a, b in zip(m["mask_start"].to_numpy().astype("datetime64[us]").astype("int64"),
            ...
            valid &= ~((ts >= a) & (ts <= b))
    # exact-repeat-run filter REMOVED
...
fig.suptitle("[Chelten] SCG vs Polar CORAL HR -- no anti-coasting, no exact-repeat-run, one 4-minute window", fontsize=12)
fig.tight_layout()
out = "/tmp/dog-test-ecg/dog-test-ecg-code/figs/chelten_window_nofilter.png"
