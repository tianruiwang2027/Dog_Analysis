...
S1S2_LO = float(__import__('sys').argv[3]) if len(__import__('sys').argv)>3 else 0.16
S1S2_HI = float(__import__('sys').argv[4]) if len(__import__('sys').argv)>4 else 0.20
ts_sd, env_sd, fsd_s = shannon_envelope_decimated(xf_s, ts_s, fs_s)
sharp_s, q995_s = sharpen_local(env_sd, fsd_s, q995_win_s=8.0)
s1_scg, s2_scg, cand_scg = pick_doublets(ts_sd, sharp_s, fsd_s, thr=SCG_THR, s1s2_lo=S1S2_LO, s1s2_hi=S1S2_HI)
...
with open(f"/tmp/shannon_restricted_results_thr{SCG_THR}_gap{S1S2_LO}-{S1S2_HI}.pkl","wb") as f:
...
print(f"saved results+diagnostics pickle (SCG_THR={SCG_THR}, ECG_THR={ECG_THR}, gap={S1S2_LO}-{S1S2_HI})")
