...
print("\n--- CNN (envelope, SNAPPED + SCG+ECG relabeled), held-out test (thr=0.5) ---")
...
torch.save(best_state, "/tmp/cnn_model_snap.pt")
with open("/tmp/cnn_train_snap_result.pkl","wb") as f:
    pickle.dump(dict(scale=scale, n_train=n_train, test_prob=test_prob, y_test=y_test, t_test=t[n_train:]), f)
print("saved /tmp/cnn_model_snap.pt")
