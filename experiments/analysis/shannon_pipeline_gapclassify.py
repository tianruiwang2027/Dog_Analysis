...
def pick_doublets_gapclassify(ts, env, fsd, refract_pk_s=0.08, gap_lo_s=0.0, gap_thr_s=0.35, thr=0.6):
    """Naive gap-classification detector: find all candidate peaks (same
    find_peaks call as pick_doublets), then just look at consecutive-candidate
    time gaps. Any gap shorter than gap_thr_s is called an S1-S2 pair (no lo/hi
    window, no post-S2 refractory beyond skipping the consumed candidate).
    Anything >= gap_thr_s is left as a gap between cycles (no beat marked)."""
    cand, _ = sg.find_peaks(env, height=thr, distance=max(1, int(refract_pk_s*fsd)))
    cand_t = ts[cand]
    s1_times = []; s2_times = []
    i = 0; n = len(cand_t)
    while i < n - 1:
        gap_s = (cand_t[i+1] - cand_t[i]) / 1e6
        if gap_lo_s <= gap_s < gap_thr_s:
            s1_times.append(cand_t[i]); s2_times.append(cand_t[i+1])
            i += 2
        else:
            i += 1
    return np.array(s1_times, dtype="int64"), np.array(s2_times, dtype="int64"), cand_t
...
SCG_THR = float(__import__('sys').argv[1]) if len(__import__('sys').argv)>1 else 0.6
ECG_THR = float(__import__('sys').argv[2]) if len(__import__('sys').argv)>2 else 0.6
GAP_LO_S = float(__import__('sys').argv[3]) if len(__import__('sys').argv)>3 else 0.0
GAP_THR_S = float(__import__('sys').argv[4]) if len(__import__('sys').argv)>4 else 0.35
ts_sd, env_sd, fsd_s = shannon_envelope_decimated(xf_s, ts_s, fs_s)
sharp_s, q995_s = sharpen_local(env_sd, fsd_s, q995_win_s=8.0)
s1_scg, s2_scg, cand_scg = pick_doublets_gapclassify(ts_sd, sharp_s, fsd_s, thr=SCG_THR, gap_lo_s=GAP_LO_S, gap_thr_s=GAP_THR_S)
...
R_e = pick_singles(ts_ed, sharp_e, fsd_e, refract_s=0.28, thr=ECG_THR)
...
with open(f"/tmp/shannon_restricted_results_thr{SCG_THR}_gapclassify{GAP_LO_S}-{GAP_THR_S}.pkl","wb") as f:
...
print(f"saved results+diagnostics pickle (SCG_THR={SCG_THR}, ECG_THR={ECG_THR}, gap_thr_s={GAP_THR_S})")
