...
def stats(a,b):
    d = a-b
    return dict(n=len(a), bias=d.mean(), mae=np.abs(d).mean(),
                within10=np.mean(np.abs(d)<=10), r=np.corrcoef(a,b)[0,1] if len(a)>2 else float('nan'))

def align_window(lag_us):
    xa, yb, ta = align(scg_tmid, scg_hr, scg_v, ecg_tmid, ecg_hr, ecg_v, lag_us=lag_us)
    inw = (ta>=t0)&(ta<=t1)
    return xa[inw], yb[inw]

print("lag scan, restricted to 17:26:30-end:")
lags = np.arange(-5,20.01,0.5)
rs = []
for lag_s in lags:
    xa,yb = align_window(int(lag_s*1e6))
    R = stats(xa,yb)
    rs.append(R['r'])
    print(f"  lag={lag_s:+6.2f}s  n={R['n']:4d}  MAE={R['mae']:.3f}  bias={R['bias']:+.3f}  r={R['r']:.4f}")
rs = np.array(rs)
best_lag = lags[np.argmax(rs)]
print("best lag:", best_lag)

xa0, yb0 = align_window(0)
R0 = stats(xa0, yb0)
xaL, ybL = align_window(int(best_lag*1e6))
RL = stats(xaL, ybL)
print("lag=0:", R0)
print(f"lag=+{best_lag:.1f}s:", RL)
...
ax.set_xlabel("lag applied to SCG timestamps (s)"); ax.set_ylabel("r (smoothed HR)")
...
ax.set_title(f"lag=0s: r={R0['r']:.3f} MAE={R0['mae']:.2f}\nbias={R0['bias']:+.2f} n={R0['n']}")
...
ax.set_xlim(lim); ax.set_ylim(lim); ax.set_aspect("equal")
ax.set_xlabel("Polar hand-beat HR"); ax.set_ylabel("SCG hand-beat HR")
ax.set_title(f"lag=+{best_lag:.1f}s: r={RL['r']:.3f} MAE={RL['mae']:.2f}\nbias={RL['bias']:+.2f} n={RL['n']}")
ax.grid(True, alpha=0.3)

fig.suptitle("[Chelten] SCG vs Polar hand-beat HR, lag-corrected, 17:26:30 to end of annotation", fontsize=13)
fig.tight_layout()
out = "/tmp/dog-test-ecg/dog-test-ecg-code/figs/chelten_lag_explain_from172630.png"
